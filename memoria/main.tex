...
Aunque si hubo un problema: en MariaDB los índices de clave primaria posteriores generados mediante un \mintinline{mysql}{ALTER TABLE} no sobreescriben los unique anteriores por lo que hubo que eliminarlos a mano para añadir los de clave primaria.
...
En el fichero \mintinline{bash}{/etc/mysql/my.cnf} dentro del contendor de MariaDB, es necesario añadir la siguiente linea donde se encuentran las sentencias de \emph{innodb} : \mintinline{bash}{innodb_page_size = <tamaño_de_página>}. En nuestro caso \mintinline{bash}{tamaño_de_página} valdrá: \mintinline{bash}{4k, 16k y 64k} (Importante la k en minúscula). El buffer ya venía preconfigurado con un tamaño razonable de 246 Mb.
